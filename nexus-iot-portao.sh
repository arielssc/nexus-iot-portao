...
# Versao: 1.3 - API fiel ao sistema IoT original
# ============================================================
...
// ================== CONFIGURAÇÃO MQTT (VPS) ==================
// Conecta ao Mosquitto local na porta definida na instalação
...
mqttClient.on('connect', () => {
    console.log('✅ API conectada ao Broker MQTT Local (__MQTT_PORT__)!');
    // Subscreve a tudo o que venha do portão
...
// ================== SERVIDOR HTTP (Rotas e Comandos) ==================
const server = http.createServer((req, res) => {
    // Configuração de CORS para o App/Navegador
    ...
    // Endpoint que o app usa para status do portão
...
io.on('connection', (socket) => {
    console.log(`📱 Telemóvel conectado no túnel: ${socket.id}`);
    // Envia o estado atual assim que o app abre
...
// ================== LÓGICA DE RECEPÇÃO MQTT ==================
mqttClient.on('message', (topic, message) => {
    const msg = message.toString().trim();

    // Log para ver no terminal o que está chegando de fato
    console.log(`📩 MQTT -> Tópico: ${topic} | Mensagem: ${msg}`);

    // Usamos .includes igual ao sistema original, para evitar problemas com barras ou espaços nos tópicos
    ...
    // Dispara a atualização para TODOS os celulares via WebSocket
...
server.listen(__API_PORT__, () => {
    console.log('🛡️ Nexus API Online na porta __API_PORT__');
});
